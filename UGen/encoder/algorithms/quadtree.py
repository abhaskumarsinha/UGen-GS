...
"""
Priority-driven Quadtree Gaussian Approximation (fixes top-left bias)

- Build initial root region and its fit (moments -> gaussian -> error).
- Use a max-heap keyed by region error_ratio = error / energy.
- Pop the worst region, either accept it as an atom (if it meets stopping
  criteria) or subdivide it into 4 children and push them on the heap.
- Continue until heap empty or max_atoms reached.

This avoids deterministic recursion order bias.
"""
...
def load_image_rgb(path, size=None):
    im = Image.open(path).convert("RGB")
    if size is not None: im = im.resize(size, Image.LANCZOS)
    return np.asarray(im).astype(np.float32) / 255.0
def save_image_rgb(arr, path):
    img = Image.fromarray((clamp01(arr) * 255.0).astype(np.uint8)); img.save(path)
def luminance_map(img):
    return 0.299*img[...,0] + 0.587*img[...,1] + 0.114*img[...,2]

# -------------------------
# Moments and gaussian fit
# -------------------------
import numpy as np
import math

# -------------------------
# Robust moments + covariance stabilization
# -------------------------

def compute_region_moments(I_patch: np.ndarray, use_luminance: bool = True):
    """
    Compute intensity-weighted moments with robust handling for tiny mass.
    Returns: mass, mu_y, mu_x, cov (2x2)
    """
    h, w, _ = I_patch.shape
    if use_luminance:
        weights = 0.299 * I_patch[...,0] + 0.587 * I_patch[...,1] + 0.114 * I_patch[...,2]
    else:
        weights = I_patch.mean(axis=2)

    mass = float(weights.sum())
    # if mass tiny, return center-of-patch and small isotropic covariance
    if mass <= 1e-8:
        mu_y = (h-1)/2.0
        mu_x = (w-1)/2.0
    ...
    # small regularization to avoid exact zeros (still might be singular if rank-1)
    cov += np.eye(2) * 1e-8
    return mass, mu_y, mu_x, cov.astype(np.float32)


def stabilize_covariance(cov: np.ndarray, min_ratio: float = 1e-3, eps_abs: float = 1e-6):
    """
    Ensure cov is symmetric positive-definite by clamping eigenvalues.
    - min_ratio: min_eig >= min_ratio * max_eig
    - eps_abs: absolute lower bound on eigenvalues
    Returns cov_pd (2x2 float64).
    """
    cov = np.array(cov, dtype=np.float64)
    # symmetrize for numeric safety
    cov = 0.5 * (cov + cov.T)
    vals, vecs = np.linalg.eigh(cov)   # ascending eigenvalues
    ...
    # final tiny regularization
    cov_pd += np.eye(2) * 1e-12
    return cov_pd

# -------------------------
# Gaussian phi using stabilized cov
# -------------------------

def gaussian_phi_patch(h: int, w: int, mu_y: float, mu_x: float, cov: np.ndarray):
    """
    Build phi over patch shape (h,w). Uses stabilized covariance (SPD).
    Returns phi (float32).
    """
    # stabilize covariance to avoid singular inversion
    cov_pd = stabilize_covariance(cov, min_ratio=1e-3, eps_abs=1e-6)
...
# -------------------------
# Hardened fitting routine
# -------------------------

def fit_gaussian_to_patch(I_patch: np.ndarray):
    """
    Robust fitting: returns dict with fields:
    mass, mu_y, mu_x, cov (stabilized), phi, p, recon, error, energy
    """
    # Compute moments
    mass, mu_y, mu_x, cov = compute_region_moments(I_patch)
    h, w, _ = I_patch.shape

    # if patch energy is zero, return trivial atom
    energy = float((I_patch ** 2).sum())
    ...
    # build phi using stabilized covariance
    phi = gaussian_phi_patch(h, w, mu_y, mu_x, cov)
    phi_sq_sum = float((phi * phi).sum())
    if phi_sq_sum <= 1e-12:
        # degenerate phi; fallback to isotropic small Gaussian centered in patch
        fallback_sigma = max(1.0, math.sqrt(max(cov[0,0], cov[1,1])))
        # isotropic fallback kernel:
        ys = np.arange(h)[:,None]; xs = np.arange(w)[None,:]
        dy = ys - mu_y; dx = xs - mu_x
        phi = np.exp(-0.5 * (dx*dx + dy*dy) / (fallback_sigma**2)).astype(np.float32)
        phi_sq_sum = float((phi*phi).sum())
        if phi_sq_sum <= 1e-12:
            # give up and return trivial fit
            return {
                'mass': mass, 'mu_y': mu_y, 'mu_x': mu_x, 'cov': cov,
                'phi': np.zeros((h, w), dtype=np.float32),
                'p': np.zeros(3, dtype=np.float32),
    ...
    # compute premultiplied color p = sum(phi * I) / sum(phi^2)
...
# -------------------------
# Priority-driven quadtree builder
# -------------------------
_region_counter = 0
def make_region_entry(y0,y1,x0,x1, depth, fit):
    # key = error_ratio (higher => pop earlier). heap stores negative for max-heap
    err_ratio = fit['error'] / (fit['energy'] + 1e-12)
    global _region_counter
    _region_counter += 1
    # entry: ( -err_ratio, counter, data_dict )
    data = { 'y0':y0,'y1':y1,'x0':x0,'x1':x1,'depth':depth,'fit':fit }
    return (-err_ratio, _region_counter, data)

def priority_quadtree(I, max_atoms=200, err_ratio_thresh=0.04, max_depth=8, min_size=8, verbose=True):
    H,W,_ = I.shape
    atoms = []

    # initial root fit
    root_fit = fit_gaussian_to_patch(I)
    heap = []
    heapq.heappush(heap, make_region_entry(0,H,0,W,0, root_fit))

    while heap and (max_atoms is None or len(atoms) < max_atoms):
        ...
        # stopping checks (local)
        if (h <= 0) or (w <= 0):
            continue
        if (depth >= max_depth) or (h <= min_size and w <= min_size) or (fit['energy'] <= 1e-12) or (err_ratio <= err_ratio_thresh):
            # accept as atom
            mu_global_y = y0 + fit['mu_y']
            mu_global_x = x0 + fit['mu_x']
            atom = {
                ...
                'p': fit['p'], 'error':fit['error'],'energy':fit['energy'],'err_ratio':err_ratio
            }
            atoms.append(atom)
            if verbose:
        ...
        # otherwise subdivide into 4 children, compute fits and push them
        ym = y0 + h // 2
        xm = x0 + w // 2
        children = [
            (y0, ym, x0, xm),
        ...
        for (cy0, cy1, cx0, cx1) in children:
            ...
            fit_child = fit_gaussian_to_patch(patch)
            entry = make_region_entry(cy0, cy1, cx0, cx1, depth+1, fit_child)
...
# -------------------------
# Rendering (same compositing rule as before)
# -------------------------
def render_atoms(atoms: List[dict], H: int, W: int):
    # sort by energy ascending (dimmer first -> background)
    atoms_sorted = sorted(atoms, key=lambda a: a['energy'])
